 ...
 * No future/integration business endpoints are invented.
 *
...
export function createOpenApiDocument(
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'Enterprise integration middleware platform. Foundation endpoints only ' +
        '(health, readiness, version, integration health, identity). External ' +
        'business integrations (FSM, FSM Scheduler, Sage X3, SQL, WorkSuite, ' +
        'Lead Perfection) are NOT implemented yet.',
    )
    .setVersion(version)
    .addBearerAuth(
      {
        ...
        scheme: 'bearer',
        bearerFormat: 'JWT',
        description: 'Bearer token issued by the configured identity provider.',
      },
      'bearer',
    )
    .addTag('Health', 'Liveness and readiness probes')
    .addTag('Version', 'Service version information')
    .addTag('Auth', 'Authenticated identity')
    .build();
